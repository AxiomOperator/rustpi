...
impl SessionStore for SqliteBackend {
    ...
    async fn insert_session_record(&self, id: &SessionId) -> Result<SessionRecord, StoreError> {
        let now = now_str();
        sqlx::query(
            "INSERT OR IGNORE INTO sessions (id, created_at, updated_at, summary) VALUES (?, ?, ?, NULL)",
        )
        .bind(id.to_string())
        .bind(&now)
        .bind(&now)
...
impl RunStore for SqliteBackend {
    ...
    async fn insert_run_record(
        ...
        let now = now_str();
        sqlx::query(
            "INSERT OR IGNORE INTO runs (id, session_id, created_at, completed_at, status) VALUES (?, ?, ?, NULL, ?)",
        )
        .bind(id.to_string())
        .bind(session_id.to_string())
        .bind(&now)
